...
public class SimpleLeftOuterJoin {
    public static void main(String[] args) throws  Exception{
        Context z3Context = new Context();
        simpleParser parser = new simpleParser();
        RelNode newNode = parser.getRelNode("SELECT * FROM EMP LEFt JOIN DEPT ON EMP.DEPTNO = DEPT.DEPTNO WHERE DEPT.DEPTNO > 10.");
        System.out.println(RelOptUtil.toString(newNode));
        AlgeNode algeExpr = AlgeNodeParserPair.constructAlgeNode(newNode,z3Context);
       // System.out.println("let's check result here");
        //System.out.println(algeExpr.getInputs().get(1));
        //System.out.println(algeExpr);
        simpleParser parser2 = new simpleParser();
        RelNode newNode2 = parser2.getRelNode("SELECT * FROM EMP LEFt JOIN DEPT ON EMP.DEPTNO = DEPT.DEPTNO WHERE DEPT.DEPTNO > 10.");
